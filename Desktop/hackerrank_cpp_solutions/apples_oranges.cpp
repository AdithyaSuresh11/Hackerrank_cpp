...
{   
    vector <int> apple_value;
    int number_of_apples = 0;
    vector <int> orange_value;
    int number_of_oranges = 0;
    vector <int> fruits;

    for (int i = 0; i < apples.size(); i++)
    {   
        apple_value.push_back(a + apples[i]);

        if (apple_value[i] >= s && apple_value[i] <= t)
        {
            number_of_apples += 1;
            // std::cout<< number_of_apples << std::endl;   
        } 
    }
    for (int j = 0; j < oranges.size(); j++)
    {   
       orange_value.push_back(b + oranges[j]);

       if (orange_value[j] >= s && orange_value[j] <= t)
       {
            number_of_oranges += 1;
            // std::cout<< number_of_oranges << std::endl;   
       } 
    }
    std::cout<< number_of_apples << std::endl;
    std::cout<< number_of_oranges << std::endl;  
